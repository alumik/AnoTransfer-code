...
class KPI:
    ...
    def split_by_idx(self, idx: int, window_size: int = 120) -> Tuple['KPI', 'KPI']:
        ...
        ret = []
        ret.append(KPI(timestamps=self.timestamps[:idx],
                       values=self.values[:idx],
                       labels=self.labels[:idx],
                       missing=self.missing[:idx],
                       name=self.name))
        # 保留前一天的 window_size - 1个点, 从而解决时间窗口的问题
        ret.append(KPI(timestamps=self.timestamps[idx-(window_size-1):],
                       values=self.values[idx-(window_size-1):],
                       labels=self.labels[idx-(window_size-1):],
                       missing=self.missing[idx-(window_size-1):],
                       name=self.name))
        return tuple(ret)

    def split_days(self, days: int, window_size: int = 120) -> Tuple[Dict, Dict, 'KPI']:
        """
        split the KPI into train_kpi[...], test_kpi[...]
        the number of train_kpi equals to `days` (depends on time interval)
        """
        total_minutes = days * 24 * 60
        inteval = self.interval / 60  # interval in minute
        num_of_point = int(total_minutes / inteval)
        train_kpi, test_kpi = self.split_by_idx(num_of_point, window_size = window_size)

        # split by day
        train_ts = train_kpi.timestamps
        train_datetime = [datetime.fromtimestamp(ts) for ts in train_ts]

        test_ts = test_kpi.timestamps
        test_datetime = [datetime.fromtimestamp(ts) for ts in test_ts]

        train_week_day_map = self._get_daily_kpi(train_kpi, train_datetime, window_size)
        test_week_day_map = self._get_daily_kpi(test_kpi, test_datetime, window_size)
        return train_week_day_map, test_week_day_map, test_kpi
    ...
    def _get_daily_kpi(self, kpi: 'KPI', datetime_seq: Sequence, window_size: int = 120):
        i = window_size
        week_map = {}
        while i < len(kpi.timestamps):
            if datetime_seq[i].day != datetime_seq[i-1].day:
                weekday = datetime_seq[i-1].weekday()
                datetime_seq = datetime_seq[i-(window_size-1):]
                dst_kpi, kpi = kpi.split_by_idx(i, window_size=window_size)
                i = window_size
                if len(dst_kpi.timestamps) > window_size:
                    if weekday not in week_map:
                        week_map[weekday] = []
                    week_map[weekday].append(dst_kpi)
            else:
                i += 1
        if len(kpi.timestamps) > window_size * 4:
            weekday = datetime_seq[0].weekday()
            if weekday not in week_map:
                week_map[weekday] = []
            week_map[weekday].append(kpi)
        return week_map
...
class KPIDataset:
    ...
    def __init__(self, kpi: KPI, window_size: int, missing_injection_rate: float = 0.):
        ...
        self._one_hot_minute = self._one_hot(
            self._ts2minute(kpi.timestamps), depth=60)
        self._one_hot_hour = self._one_hot(
            self._ts2hour(kpi.timestamps), depth=24)
        self._one_hot_weekday = self._one_hot(
            self._ts2weekday(kpi.timestamps), depth=7)
        ...
        for i in range(len(self._value_windows)):
            ...
            time_code = np.concatenate(
                [self._one_hot_minute[time_index], self._one_hot_hour[time_index],
                    self._one_hot_weekday[time_index]],
                axis=-1
            )
    ...
    def _ts2weekday(ts: np.ndarray) -> np.ndarray:
        return np.zeros_like(((ts // 86400) + 4) % 7)
    ...
    def to_torch(self, device: str) -> TensorDataset:
        return TensorDataset(torch.as_tensor(self.values, device=torch.device(device)),
                             torch.as_tensor(
                                 self.time_code, device=torch.device(device)),
                             torch.as_tensor(self.normal, device=torch.device(device)))
